...
SpriteAnimator::SpriteAnimator(std::shared_ptr<Sprite2D> sprite_ptr)
    : m_animated_sprite(sprite_ptr), m_currnet_sprite(std::move(sprite_ptr)) {
  m_current_state = m_animated_sprite->m_states.end();
}

void SpriteAnimator::setState(const std::string& state_name) {
  Sprite2D::MapStates::const_iterator state =
      m_animated_sprite->m_states.find(state_name);
...
double SpriteAnimator::getDurationAnimation(
...
void SpriteAnimator::Update(const double delta) {
  if (m_current_state != m_animated_sprite->m_states.end()) {
    ...
    while (m_current_time_animation >=
           m_current_state->second[m_current_frame].second) {
      m_current_time_animation -=
          m_current_state->second[m_current_frame].second;
      ++m_current_frame;
      m_is_need_rerender = true;
...
void SpriteAnimator::Render(const glm::vec2& position, const glm::vec2& size,
  ...
  if (m_is_need_rerender) {
    m_currnet_sprite =
        BatleCity::Game::Instance().GetResourcesManager()->GetSprite(
