...
Sprite2D::Sprite2D(std::shared_ptr<Texture2D> ptr_texture,
    ...
    : IRenderObjectRect2D(std::move(ptr_shader_program)),
      m_texture(std::move(ptr_texture)) {
  RenderEngine::Texture2D::SubTexture2D subTexture =
      m_texture->getSubTexture(std::move(subTexture_name));
...
bool Sprite2D::isAnimated() const { return !m_states.empty(); }

void Sprite2D::addState(const std::string& state_name,
                        std::vector<std::pair<std::string, double>>&& frames) {
  m_states.emplace(state_name, std::move(frames));
}

double Sprite2D::getDurationStateAnimation(
