...
AnimatedSprite2D::AnimatedSprite2D(
    const std::shared_ptr<Texture2D> ptr_texture,
    const std::shared_ptr<ShaderProgram> ptr_shader_program,
    const std::string& subTexture_name)
    : Sprite2D(ptr_texture, ptr_shader_program, subTexture_name),
      m_current_frame(0),
...
void AnimatedSprite2D::addState(
...
void AnimatedSprite2D::render(const glm::vec2& position, const glm::vec2& size,
  ...
  if (m_need_rerender) {
    RenderEngine::Texture2D::SubTexture2D current_subTexture =
        m_texture->getSubTexture(
            m_current_state->second[m_current_frame].first);

    GLfloat texture_coords[] = {
        //					U
        // V
        current_subTexture.left_bottom_uv.x,
        current_subTexture.left_bottom_uv.y,
        current_subTexture.left_bottom_uv.x,
        current_subTexture.right_top_uv.y,
        current_subTexture.right_top_uv.x,
        current_subTexture.right_top_uv.y,
        current_subTexture.right_top_uv.x,
        current_subTexture.left_bottom_uv.y,
    };
