...
namespace BatleCity {
BrickWall::BrickWall(const EBlockType wall_type, const glm::vec2& position,
...
BrickWall::ESubBlockLocation BrickWall::GetSubBlockLocationByCollider(
  ...
  if (collider->getLeftBottom().x >= 0.f &&
      ...
      collider->getRightTop().y > 0.f &&
      collider->getRightTop().y <= m_size.y / 2.f) {
    return ESubBlockLocation::LeftBottom;
  } else if (collider->getLeftBottom().x >= m_size.x / 2.f &&
             collider->getLeftBottom().x < m_size.x &&
             collider->getRightTop().y > m_size.y / 2.f &&
             collider->getRightTop().y <= m_size.y) {
    return ESubBlockLocation::RightTop;
  } else if (collider->getLeftBottom().x >= m_size.x / 2.f &&
             collider->getLeftBottom().x < m_size.x &&
             collider->getRightTop().y > 0 &&
             collider->getRightTop().y <= m_size.y / 2.f) {
    return ESubBlockLocation::RightBottom;
  } else if (collider->getLeftBottom().x >= 0.f &&
             collider->getLeftBottom().x < m_size.x / 2.f &&
             collider->getRightTop().y > m_size.y / 2.f &&
             collider->getRightTop().y <= m_size.y) {
