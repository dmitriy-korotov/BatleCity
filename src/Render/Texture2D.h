...
class Texture2D {
 public:
  ...
  Texture2D(const GLboolean* pixels, const GLuint width, const GLuint height,
            const GLuint chahhels = 4, const GLuint filter = GL_LINEAR,
            const GLuint wrap_mode = GL_CLAMP_TO_EDGE);
  ~Texture2D();
