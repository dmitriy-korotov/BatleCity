...
std::optional<EnemyTankAI::Path> EnemyTankAI::calculatePathToEagle()
  ...
  if (m_eagle_position.has_value()) {
    ...
    auto [dp, last_visited_point] =
        getFilledDynamic(_indexes_tank_position, m_eagle_position);
...
EnemyTankAI::Point EnemyTankAI::getIndexesTankPosition() const noexcept {
  size_t x_index = static_cast<size_t>(
      std::round((m_enemy_tank->getPosition().x - m_level->getLeftOffset()) /
                 m_level->getBlockSize()));
  size_t y_index = static_cast<size_t>(
      std::round((m_level->getGameStateHeight() - m_level->getTopOffset() -
                  m_enemy_tank->getSize().y - m_enemy_tank->getPosition().y) /
                 m_level->getBlockSize()));
  return {x_index, y_index};
}

std::pair<std::vector<std::vector<int64_t>>, EnemyTankAI::Point>
EnemyTankAI::getFilledDynamic(
  ...
  std::vector<std::vector<int64_t>> dp(
      level_height, std::vector<int64_t>(level_width, INT_MAX));
  ...
  while (!_points_witch_need_visit.empty()) {
    ...
    if (_current_point.first >= 1 && _current_point.second >= 1) {
      dp[_current_point.second][_current_point.first] =
          std::min(dp[_current_point.second][_current_point.first - 1],
                   dp[_current_point.second - 1][_current_point.first]) +
          1;
    } else if (_current_point.first >= 1) {
      dp[_current_point.second][_current_point.first] =
          dp[_current_point.second][_current_point.first - 1] + 1;
    } else if (_current_point.second >= 1) {
      dp[_current_point.second][_current_point.first] =
          dp[_current_point.second - 1][_current_point.first] + 1;
    }

    if (level_description[_current_point.second][_current_point.first] ==
        EAGLE_SYMBOL) {
      break;
    }

    if (_current_point.first < eagle_position->first &&
        level_description[_current_point.second][_current_point.first + 1] ==
            PERMITTED_FOR_PATH_SYMBOL) {
      _points_witch_need_visit.emplace_front(_current_point.first + 1,
                                             _current_point.second);
    }
    if (_current_point.second < eagle_position->second &&
        level_description[_current_point.second + 1][_current_point.first] ==
            PERMITTED_FOR_PATH_SYMBOL) {
      _points_witch_need_visit.emplace_front(_current_point.first,
                                             _current_point.second + 1);
    }
  }
...
EnemyTankAI::Path EnemyTankAI::getPathFromDynamic(
    std::vector<std::vector<int64_t>>& dp, const Point& last_visited_point,
    const Point& tank_position) const noexcept {
  Path _result;
  for (int64_t x = last_visited_point.first, y = last_visited_point.second;
       y > tank_position.second || x > tank_position.first;) {
    if (x == tank_position.first) {
      _result.emplace_back(x * m_level->getBlockSize(),
                           (y--) * m_level->getBlockSize());
...
void EnemyTankAI::update(double delta) noexcept {
  if (m_enemy_tank && m_path_to_eagle.has_value() &&
    ...
    int64_t _transformed_vertical_position =
        std::ceil(m_level->getGameStateHeight() - m_enemy_tank->getSize().y -
                  m_enemy_tank->getPosition().y - m_level->getTopOffset());
    int64_t _transformed_horisontal_position =
        std::ceil(m_enemy_tank->getPosition().x - m_level->getLeftOffset());

    if (_transformed_horisontal_position <
        m_path_to_eagle->at(m_current_path_index).first) {
    ...
    switch (m_enemy_tank->getOrientation()) {
      case EnemyTank::EOrientation::Bottom:
        if (_transformed_vertical_position >=
            m_path_to_eagle->at(m_current_path_index).second) {
    ...
    if (m_enemy_tank && m_path_to_eagle.has_value()) {
      if (m_eagle_position->first * m_level->getBlockSize() ==
          m_path_to_eagle->back().first) {
        m_enemy_tank->setOrientation(EnemyTank::EOrientation::Bottom);
      } else if (m_eagle_position->second * m_level->getBlockSize() ==
                 m_path_to_eagle->back().second) {
        m_enemy_tank->setOrientation(EnemyTank::EOrientation::Right);
      }
