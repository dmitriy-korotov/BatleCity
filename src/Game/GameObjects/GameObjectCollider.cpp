...
GameObjectCollider::GameObjectCollider(
    ...
    : IRenderObjectRect2D(std::move(shader_program)), m_color(color) {
  GLfloat colors[] = {color.r, color.g, color.b, color.r, color.g, color.b,
...
void GameObjectCollider::render(const glm::vec2& position,
  ...
  for (const auto& rect_collider : m_rect_colliders) {
    if (rect_collider.isActive()) {
      ...
      model_matrix = glm::translate(
          model_matrix, glm::vec3(0.5f * (rect_collider.getRightTop() -
                                          rect_collider.getLeftBottom()),
                                  0.f));
